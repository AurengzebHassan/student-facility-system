...
<body>
    ...
    <div class="header-section" style="margin-top: 4.7px;">
            ...
            <div class="container-fluid">
                <div class="row mt-2">
                    ...
                    <div class="col-md-7 d-flex align-items-center justify-content-center hide-on-small-screens">
                        ...
                        <a href="{{ route('frontend.menu')}}" class="nav-link active font-weight-bold"
                            style="border-bottom: 2px solid #ff6826; color: #ff6826">Menu</a>
        ...
        ">
            ...
            <aside class="sidebar d-none">
                ...
                <a href="{{ route('frontend.menu') }}" class="nav-link nav-links active font-weight-bold"
                    style="border-bottom: 2px solid #ff6826; color: #ff6826;">Menu</a>
                ...
                <a class="nav-link font-weight-bold nav-links" href="{{ route('userprofile')}}"
                    style="color: #000000;">Profile</a>
                <a class="nav-link font-weight-bold nav-links" href="{{ route('orderhistory')}}"
                    style="color: #000000;">Order History</a>
                <a class="nav-link font-weight-bold nav-links" href="{{ route('user.logout') }}"
                    style="color: #000000;">Logout</a>
                @else
                <!-- Show login button when session does not have 'id' data -->
                <a class="nav-link font-weight-bold nav-links " href="{{ route('login') }}"
                    ...
                    <div class="form search-form">
                        <form action="{{ route('frontend.menu') }}" method="get">
                            <div class="input-group">
                                <input type="text" id="searchInput" style="border-radius: 20px;"
                                    class="form-control form-input" name="search" value="{{ $searchQuery }}"
                                    placeholder="Search anything...">
                                <div class="input-group-append">
                                    <button style="border-radius: 8px;" type="submit"
                                        class="btn menu-search-button">Search</button>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <div class="items-section">
            ...
            <div class="row">
              <div class="items-btn" id="categorySlider">
                  <a href="{{ route('frontend.menu') }}">
                      <span type="button" class="btn active menu-items-btn">All Items</span>
                  </a>
                  {{-- Other menu item buttons --}}
                  @foreach ($categories as $categoryItem)
                      <a href="{{ route('frontend.menu.category', ['category' => $categoryItem->id]) }}">
                          <span type="button" class="btn {{ $category && $category->id == $categoryItem->id ? 'active' : '' }} menu-items-btn">{{ $categoryItem->name }}</span>
                      </a>
                  @endforeach
                ...
                <div class="col-lg-4 col-md-6 col-sm-12 mx-auto text-center">
                    <div class="card menu-card">
                        <img src="{{ asset('product/images/' . $product->image) }}"
                            class="card-img-top mx-auto product-image" alt="{{ $product->name }}">
                        ...
                        @endif --}}
                            ...
                            <a id="orderButton" href="{{ route('confirmationorder', ['uuid' => $product->uuid]) }}"
                                class="buy-now-btn" data-uuid="{{ $product->uuid }}">Order Now</a>
                            @else
                            <button type="button" class="btn buy-now-btn text-center" disabled>Order Now (Login
                                required)</button>
                            @endif
                            @else
                            <button type="button" class="btn buy-now-btn text-center" disabled>Order Now (Login
        ...
        <div class="container mt-5">
            <div class="row">
              <div class="btn-explore text-center mx-auto">
              <button type="button" class="btn more-btn" id="loadMoreBtn">Load more</button>
            </div>
            </div>
          </div>
    </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-lg-3">
                    ...
                    <div class="form-group">
                        <input type="email" class="form-control footer-input-field" autocomplete="off" id="email"
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Define the number of cards to show initially and per click
  var initialCardsToShow = 12;
  var cardsPerClick = 3;

  // Function to show additional cards with a delay
  function showNextCards(startIndex, endIndex) {
    setTimeout(function () {
  ...
  for (var i = initialCardsToShow; i < menuCards.length; i++) {
    menuCards[i].style.display = "none";
  }

  // Get the "Load more" button
  var loadMoreBtn = document.querySelector(".more-btn");

  // Add click event listener to the "Load more" button
  loadMoreBtn.addEventListener("click", function () {
    // Calculate the next set of cards to display
    var startIndex = initialCardsToShow;
    var endIndex = startIndex + cardsPerClick;

    // Show the next set of cards with a delay
    showNextCards(startIndex, endIndex);

    // Update the initial cards to show for the next click
    initialCardsToShow = endIndex;

    // Hide the "Load more" button if all cards are displayed
    // if (initialCardsToShow >= menuCards.length) {
