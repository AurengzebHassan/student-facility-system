        ...
        <div class="details">
            <div class="recentOrders">
                <div class="cardHeader">
                    <h2>New User</h2>
                    <form action="{{route ('user.profile')}}" method="GET" class="form-inline">
                    ...
                    <tbody>
                        ...
                        <tr style="background-color: {{ $loop->iteration % 2 == 0 ? '#f2f2f2' : 'transparent' }}">     
                            <td style="padding-right: 50px;" class="fw-bold">{{ $user->name }}</td>
                                ...
                                <td > <a href="{{ route('editAuthenticated.user', ['id' => $user->id]) }}" class="btn btn-info btn-sm" onclick="disableLink(event, this)">Edit</a></td>
                                <td>
                                   
                                    @if($user->is_enabled == 0)
                                        <a href="{{ route('enable.user', ['id' => $user->id]) }}" class="btn btn-primary btn-sm" onclick="disableLink(event, this)">Enable</a>
